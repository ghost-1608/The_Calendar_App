        ...
        for day in range(7):
            ...
            if dd == day_no and month_no == 1 and mm == TODAY.month and yyyy == TODAY.year:
                if has_event((yyyy, mm, day_no)):
                    tk.Button(week_frames[week + 1], text=disp, state=state, bg='#48D', fg='white',
                              font=('Courier', 12, 'underline'), width=3, height=1,
                              command=partial(set_date_selected, yyyy, mm, day_no)).pack(side='left', expand=True,
                                                                                         fill='both')
                else:
                    tk.Button(week_frames[week + 1], text=disp, state=state, bg='#48D', fg='white',
                              font=('Courier', 12), width=3, height=1,
                              command=partial(set_date_selected, yyyy, mm, day_no)).pack(side='left', expand=True,
                                                                                         fill='both')
                continue
...
def event_select(event=None):
    ...
    tk.Label(popup, text=eval(value[-1])[0] + ' (' + str(date_selected) + ')', font=('Courier', 14, 'bold')).pack(
        side='top', fill='x')
    ...
    tk.Button(popup, text='Delete Event', bg='#F21', fg='#FFF', font=('Courier', 12),
              command=lambda: delete_event(eval(value[0]), eval(value[1]), popup)).pack(side='bottom')
    popup.mainloop()
...
def about():
   ...
   /_/_/_/_/_/\___/_    __               __             ___              
     / ____/___ _/ /___/ /__  ____  ____/ /__  _____   /   |  ____  ____ 
    / /   / __ `/ / __  / _ \/ __ \/ __  / _ \/ ___/  / /| | / __ \/ __ \
   / /___/ /_/ / / /_/ /  __/ / / / /_/ /  __/ /     / ___ |/ /_/ / /_/ /
   \____/\__,_/_/\__,_/\___/_/ /_/\__,_/\___/_/     /_/  |_/ .___/ .___/ 
                                                          /_/   /_/      
